...
runFolder = f"runtheta={rHole},nHoles={int(numHoles/8)}" #
l1 = 14
l2 = l1+width
holeCentres = [l1+width/2 ]#+5*i for i in range(4)]
...
ax[0].set_title("Porosity measure $\Lambda(z)$")
ax[1].set_title("Pressure along centreline mPa")
ax[2].set_title("Cross section flux mm$^3$/s")
ax[1].plot(xx, pressureData,"r",linewidth=3, label = "Numerical integral")
ax[1].set_ylabel("Pressure along centreline (mPa)")

#ax[0].scatter([0], [pBack-pOutlet], s=300, color="k", label = "CFD data")
ax[2].plot(xx,flux,"b",linewidth=3, label = "Numerical integral")
ax[2].set_ylabel("Cross section flux (mm$^3$/s)")
...
ax[1].scatter(xx, [pr - p[-1] for pr in p],color="r", label = "Analytic expression")#,linewidth=3)
ax[2].scatter(xx[1:],flux,color="b",  label="Analytic expression")#,linewidth=3)
"""
#-----------------------

### If centreline data is present
from outletFromOpenFOAM import extractValueOutlet
extractValueOutlet(f"{runFolder}/foam.foam", "outlet.csv")
...
for a in ax.flatten():
    a.legend()
    a.set_xlabel("Centreline z co-ordinate (mm)")
plt.savefig("numericalVsAnalytics.png")
